...
RUN set -eux; \
    ...
    printf '%s\n' \
      ...
      'ProxyPass        "/tools/" "http://digitalinsightlive.railway.internal:3000/tools/" retry=0 timeout=60' \
      'ProxyPassReverse "/tools/" "http://digitalinsightlive.railway.internal:3000/tools/"' \
      '<Directory "/var/www">' \
      '    Require all granted' \
      '</Directory>' \
