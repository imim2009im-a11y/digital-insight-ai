...
function trimLogFile(logPath: string, maxSize: number) {
  try {
    ...
    /* diagnostics must never break development */
  }
}
...
function vitePluginManusDebugCollector(): Plugin {
  return {
    ...
    transformIndexHtml(html) {
      if (!ENABLE_MANUS_DEBUG) return html;
      return {
        html,
    ...
    configureServer(server: ViteDevServer) {
      ...
      server.middlewares.use("/__manus__/logs", (req, res, next) => {
        ...
        if (reqBody && typeof reqBody === "object") {
          try { handlePayload(reqBody); } catch { res.writeHead(400); res.end(JSON.stringify({ success: false })); }
          return;
        }
        let body = "";
        req.on("data", (chunk) => { body += chunk.toString(); });
        req.on("end", () => {
          try { handlePayload(JSON.parse(body)); } catch { res.writeHead(400); res.end(JSON.stringify({ success: false })); }
        });
      });
    },
  };
...
export default defineConfig({
  ...
  build: { outDir: path.resolve(import.meta.dirname, "dist/public"), emptyOutDir: true, rollupOptions: { output: { manualChunks(id) { if (!id.includes("node_modules")) return; if (id.includes("react-dom") || id.includes("/react/") || id.includes("scheduler")) return "react-vendor"; if (id.includes("lucide-react")) return "icons-vendor"; if (id.includes("@radix-ui") || id.includes("class-variance-authority") || id.includes("tailwind-merge")) return "ui-vendor"; if (id.includes("wouter")) return "router-vendor"; return "vendor"; } } } },
